...
void LBFGSB::updateB()
{
  ...
  if (ys > (eps * yy))
  {
    ...
    myTime += std::chrono::duration_cast<duration>(timer::now() - start);
...
void LBFGSB::cauchyPoint()
{
  ...
  for (; it != heap_end; --heap_end)
  {
    ...
    deltaTMin = (fabs(f1) < eps && fabs(f2) < eps) ? 0 : -f1/f2;  // TODO think about just f2 == 0
    Index b = it->second;
...
int LBFGSB::lineSearchMT(const Vector& deltaX, size_t iter, Scalar& stp)
{
  const Scalar INF = std::numeric_limits<Scalar>::infinity();
  // we consider a function value less than this as unbounded
  const Scalar bigMinus = -1E100;
  double a1, a2;
  double big  = 1.0e10;
  double ftol = 1.0e-3,
         gtol = 0.9,
    ...
    for (Index i = 0; i < _x.size(); ++i)
    {
      ...
      if (_lb[i] != -INF || _ub[i] != INF)
      {
        if (a1 < 0.0 && _lb[i] != -INF)
        {
          ...
          {
            stpmx = a2/a1;
            // can really happen due to roundoff errors
//            if (a1 * stpmx + _x(i) < _lb(i))
              //stpmx = std::nextafter(stpmx, 0.0);
          }
        }
...
//              stpmx = std::nextafter(stpmx, 0.0);
  ...
  int funIter = 0;
//  std::cout << "stpmx = " << std::setprecision(56) << stpmx << std::endl;
...
//  std::cout << "deltaX = " << deltaX.transpose() << std::endl;
  ...
  do
  {
    ...
    if (csave == TaskType::WARNING)
    {
        printf("Warning\n");
        return NUM_ERROR;
    }
    _xNew = stp * deltaX + _x;
    // can happend due to roundoff errors
    if (_constrained)
      projectFeasible(_xNew);
//    std::cout << "1asking at x = " << _xNew.transpose() << std::endl;
    _genoNLP.functionValueAndGradient(_xNew.data(), _fNew, _gNew.data());
//    std::cout << "f = " << _fNew << " g = " << _gNew.transpose() << std::endl;
    ++_funEval;
    if (_fNew < bigMinus)
    {
//        printf("problem unbounded\n");
    ...
    if (hasNanInf(_fNew) || hasNanInf(_gNew))
    {
      printf("encountered INF or NAN in function value or gradient\n");
      printf("zooming back into feasible region\n");
      int i = 0;
      while(hasNanInf(_fNew) || hasNanInf(_gNew))
      {
        stp *= 0.5;

        _xNew = stp * deltaX + _x;
//        std::cout << "2asking at x = " << _xNew.transpose() << std::endl;
        _genoNLP.functionValueAndGradient(_xNew.data(), _fNew, _gNew.data());
        ++_funEval;
        ++i;
        if (i>30)
        {
          printf("cannot find any feasible point\n");
          return NUM_ERROR;
        }
      }
      stpmx = stp;
    ...
    if (funIter >= 19)
    {
      // check if substantial progress was made, in this case everything is fine
      // could not satisfy curvature condition but decrease in f is properly
      // most likely the problem is unbounded
      double eps = 10.0;
      if (_fNew <= _f - eps*(std::fabs(_f) + 1))
        return UNBOUNDED;

      return NUM_ERROR;  // something went wrong
    }
  }
  while(true);
//  return SOLVED;
...
void LBFGSB::projectFeasible(Vector& v)
{
...
bool LBFGSB::hasNanInf(Scalar f)
{
  Scalar const INF = std::numeric_limits<Scalar>::infinity();
  if ((-INF < f) && (f < INF))
    return false;
...
bool LBFGSB::hasNanInf(const Vector& g)
{
  Scalar const INF = std::numeric_limits<Scalar>::infinity();
  Scalar sum = g.sum();
  if ((-INF < sum) && (sum < INF))
...
SolverStatus LBFGSB::solve()
{
  ...
  _funEval = 1;
  // std::cout << " start fg" << std::endl;
  _genoNLP.functionValueAndGradient(_x.data(), _f, _g.data());
  // std::cout << "end fg" << std::endl;
  //  std::cout << "1x " << _x.transpose() << std::endl;
  // std::cout << "f " << _f << std::endl;
  // std::cout << "g " << _g.transpose() << std::endl;
  // check for NANs and INFs
  if (hasNanInf(_f) || hasNanInf(_g))
  {
    printf("encountered INF or NAN in function value or gradient\n");
    return INIT_POINT_ERROR;
  }
  projectedGradient();
  ...
  // Backup the old state
    ...
    auto startFormK = timer::now();
    formK();
    timeFormK += std::chrono::duration_cast<duration>(timer::now() - startFormK);

    auto startSubsm = timer::now();
    subsm();
    timeSubsm += std::chrono::duration_cast<duration>(timer::now() - startSubsm);

    Vector deltaX = _xSubspace - _x;
    if (hasNanInf(deltaX))
    {
      printf("encountered INF or NAN internally in solver\n");
      return NUM_ERROR;
    }

    Scalar t = 0;
    auto startLS = timer::now();
    int ls = lineSearchMT(deltaX, _iter, t);
    auto endLS = timer::now();
    timeLS += std::chrono::duration_cast<duration>(endLS - startLS);
//    printf("\nls = %d\n", ls);
    if (ls == NUM_ERROR)
    {
      if (0 != _S.cols())
    ...
    auto startHelper = timer::now();
    Scalar deltaF = helper();
    timeHelper += std::chrono::duration_cast<duration>(timer::now() - startHelper);

    if (_verbose)
    {
      printf(" %8zu %8zu %14.5g %14.5E %14.5E\n",
             _iter, _funEval, t, _fNew, _projectedGradient.lpNorm<Eigen::Infinity>());
    }

    if (ls == UNBOUNDED)
    {
      if (_verbose)
        printf("Problem seems unbounded\n");
      return UNBOUNDED;
    }
    ...
    {
      auto end = timer::now();
      auto time_span = std::chrono::duration_cast<duration>(end - start);
      /*
      std::cout << "full c++ took " << time_span.count() << " sec" << std::endl;
      std::cout << "LS   c++ took " << timeLS.count() << " sec" << std::endl;
      std::cout << "Formkc++ took " << timeFormK.count() << " sec" << std::endl;
      std::cout << "subsmc++ took " << timeSubsm.count() << " sec" << std::endl;
      std::cout << "helprc++ took " << timeHelper.count() << " sec" << std::endl;
      std::cout << "upB  c++ took " << timeB.count() << " sec" << std::endl;
      auto total = timeLS.count() + timeFormK.count() + timeSubsm.count()
        + timeHelper.count() + timeB.count();
      std::cout << "totalc++ took " << total << " sec" << std::endl;
      std::cout << "rest c++ took " << time_span.count() - total << " sec" << std::endl;
      std::cout << "special  took " << myTime.count() << " sec" << std::endl;
    */
    ...
    if (_genoNLP.callback(_x.data()))
    {
      if (_verbose) printf("Stopped by callback\n");
      return CALLBACK_STOP;
    }
    auto startB = timer::now();
    updateB();
    timeB += std::chrono::duration_cast<duration>(timer::now() - startB);
  }
}
...
const Scalar* LBFGSB::g() const
{
  return _projectedGradient.data();
}
...
int LBFGSB::iter() const
{
  return (int) _iter;
}

int LBFGSB::funEval() const
{
  return (int) _funEval;
}
